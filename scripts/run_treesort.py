...
# Example: 'TS_NODE_9':0.00702[&rea="NA(124)",is_reassorted=1]
TREE_FILE_TS_NODE_REGEX = r"(?P<id>'TS_NODE_\d+')\s*:\s*(?P<branch>[+-]?\d+(\.\d+)?([eE][+-]?\d+)?)\s*(?P<comment>\[&rea=\"(?P<segment>\??[A-Za-z]{1,2})\((?P<distance>\d+(\.\d+)?)\)\"\s*,\s*is_reassorted=1\])?"

# Example: 'A/swine/North_Carolina/A01894897/2016|1B.2.1|2002B|TTTTPT|2016-04-06':0.00198[&rea="NA(169)",is_reassorted=1]
TREE_FILE_STRAIN_NAME_REGEX = r"'(?P<name>(?!TS_NODE_)[^':]+)'\s*:\s*(?P<branch>[+-]?\d+(\.\d+)?([eE][+-]?\d+)?)\s*(?P<comment>\[&rea=\"(?P<segment>\??[A-Za-z]{1,2})\((?P<distance>\d+(\.\d+)?)\)\"\s*,\s*is_reassorted=1\])?"

# A strain name that isn't within single quotes that's preceded by '(' or ',' and followed by a colon and number (the branch length).
TREE_FILE_UNQUOTED_NAME_REGEX = r"(?P<start>[\(,]{1})(?!'\")(?!TS_NODE_)(?P<name>[^'\":\(\)\[\]]+)\s*:\s*(?P<branch>[+-]?\d+(\.\d+)?([eE][+-]?\d+)?)\s*(?P<comment>\[[^\]]+\])?"
