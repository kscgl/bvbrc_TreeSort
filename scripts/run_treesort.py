...
class TreeSortRunner:
   ...
   def create_summary_html(self) -> bool:
      ...
      js_variables = {
         ...
         "{{result_filename}}": f"{self.job_data.output_file}{Constants.TREE_FILE_EXTENSION}",
   ...
   def process_phyloxml_clade(self, m: re.Match):
      ...
      if name.startswith("TS_NODE_"):
         ...
         properties += f'<property ref="ts:NodeID" datatype="xsd:string" applies_to="node">{name}</property>'
         name = ""
         ...
         if property and len(property) > 0:
            ...
            if reassorted_match and reassorted_match.group("reassorted") == "1":
               ...
               for sd in PHYLOXML_REGEX.SEGMENT_AND_DIST.finditer(property):
                  ...
                  properties += (
                     f'<property ref="ts:Segment" datatype="xsd:string" applies_to="node">{segment}</property>'
                     f'<property ref="ts:Distance" datatype="xsd:string" applies_to="node">{distance}</property>'
                     f'<property ref="ts:Reassorted" datatype="xsd:string" applies_to="node">yes</property>'
                     f'<property ref="ts:Uncertain" datatype="xsd:string" applies_to="node">{is_uncertain}</property>'
                  )   
            else:
               properties += (
                  f'<property ref="ts:Reassorted" datatype="xsd:string" applies_to="node">no</property>'
               )
